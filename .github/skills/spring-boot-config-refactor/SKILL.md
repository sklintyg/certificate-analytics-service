...
**Before doing anything else**, ask the user for the Jira issue ID to use as a commit message prefix:

```
What Jira issue ID should I use as the prefix for commit messages? (e.g. K1J-1525)
```

Store the answer as `{JIRA_ID}` and prefix every commit message with `{JIRA_ID}: `, for example:

```
K1J-1525: refactor(config): convert .properties to .yml (1:1, no renames)
```
...
- Prefer **one commit per phase** with passing tests at each step, prefixed with the Jira ID (`{JIRA_ID}: `).
